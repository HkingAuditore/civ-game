...
## [2.1.6] - 2026-03-20 - 部长扩建节奏重构与连续操作修复 🏛️🛠️

### 🐛 问题修复

#### 连续快速操作导致花费异常
- **问题**: 玩家在短时间内连续建造、升级、研究或征兵时，部分操作会读取到旧状态，出现花费判断错误或重复扣费
- **修复**: `useGameActions` 为资源、建筑和升级状态引入最新值 `ref`，关键操作统一从最新状态读取并在扣费后立即回写
- **影响**: ✅ 连点建造/升级/科研/招募时，资源结算更稳定，不再因为 React 批处理读到陈旧闭包

#### 自动补兵无法支援前线军团
- **问题**: 处于 `in_combat` 状态的军团会被自动补兵系统直接跳过，导致前线长期缺编
- **修复**: 补兵需求聚合和目标分配阶段不再过滤战斗中的军团，仅跳过已销毁或关闭自动补兵的军团
- **影响**: ✅ 前线军团也能持续得到补员，自动补兵终于覆盖实战场景

#### 改革法令税收标签显示异常
- **问题**: 改革法令效果文案中 `taxIncome` 标签存在重复定义，显示文本不统一
- **修复**: 统一改为“税收加成”
- **影响**: ✅ 法令效果说明更清晰，避免玩家误解
...
#### 部长自动扩建冷却重构
- **改动**: 自动扩建从单一全局冷却改为“全局检查冷却 + 分部长个人冷却”
- **具体规则**:
  - 全局检查冷却：`10` 天缩短为 `5` 天
  - 个人冷却：农业、工业、商业、市政部长各自 `10` 天
  - 旧存档中的单个数字冷却值会自动迁移为对象结构
- **效果**: ✅ 一个部长扩建后不再把其他部长一起锁死，扩建节奏更平滑，部门分工更合理
...
- **改动**: 新增最低叛乱人口门槛 `10`
- **规则**:
  - 人口低于 `10` 的阶层，组织度上限压到 `50%`
  - 若已进入叛乱触发阶段，则改为少量人口愤怒出走并带走对应财富
- **效果**: ✅ 避免 1 人、2 人阶层也能掀起叛乱的失真情况，组织度与人口规模更匹配

#### 若干 UI 与命名调整
- 国家详情页战争状态下的“求和”按钮改为正确触发 `peace` 动作
- 理念卡牌现在始终显示稀有度标签
- 已装备理念支持直接打开详情底部面板
- 时代演进面板直接读取 `EPOCHS` 名称，工业时代主题名调整为“蒸汽时代”
- 工匠阶层在低中富裕度下的 `tools` 消费需求小幅下调

---

## [2.0.9] - 2026-03-17 - 停战流程修复与 AI 时代平衡 🛠️⚖️

### 🐛 问题修复

#### 停战提议接受流程统一
- **问题**: 接受敌方停战提议时，没有统一走战争结束逻辑，可能导致后续外交状态处理不完全一致
- **修复**: 停战接受现在优先复用 `handleEnemyPeaceAccept`，统一结束战争、关系修正与日志写入流程
- **影响**: ✅ 停战结果更稳定，减少战争状态残留风险

#### 负成本价保护
- **问题**: 当营业税为负值（补贴）时，建筑总成本可能被拉到 0 以下，进一步扰乱成本价和市场价格模型
- **修复**: 建筑成本价增加下限保护，`costPrice = Math.max(0, totalCost / outputAmount)`
- **影响**: ✅ 价格模型更稳定，补贴不再制造异常负价格成本

### ⚖️ 平衡调整

#### AI 时代推进减速
- 提高 AI 各时代科技与文化需求缩放系数
- 时代升级冷却由 `200` 提升到 `600`
- 科技/文化累积速率缩放由 `1.0` 下调到 `0.4`
- **效果**: ✅ AI 不再过早冲到高时代，时代节奏更接近玩家发展曲线
