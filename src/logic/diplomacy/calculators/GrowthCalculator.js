...
export class GrowthCalculator {
    ...
    static calculateWealthGrowth({
        ...
        const epochTargetPerCapita = getTargetPerCapitaWealth(epoch);
        const perCapitaCap = getPerCapitaWealthCap(epoch);
        const playerPerCapita = playerWealth / Math.max(1, playerPopulation);
        const targetPerCapita = playerPerCapita > 0
            ? Math.min(perCapitaCap * 0.85, Math.max(epochTargetPerCapita, epochTargetPerCapita * 0.45 + playerPerCapita * 0.55))
            : epochTargetPerCapita;
        ...
        if (currentWealth < targetWealth) {
            ...
            catchUpFactor = 1.0 + Math.min(0.9, gapRatio * 0.9);
