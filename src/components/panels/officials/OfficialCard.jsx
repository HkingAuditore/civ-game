        ...
        return Object.entries(effects).map(([type, valueOrObj]) => {
            if (typeof valueOrObj === 'object' && valueOrObj !== null) {
                return Object.entries(valueOrObj).map(([target, value]) => {
                    ...
                    return (
                        <div key={`stance-${type}-${target}`} className={`text-xs ${isActive ? 'text-green-300' : 'text-red-300'}`}>
            ...
            return (
                <div key={`stance-${type}`} className={`text-xs ${isActive ? 'text-green-300' : 'text-red-300'}`}>
        ...
        >
            ...
            <div className="flex justify-between items-start mb-2 pt-1 gap-2">
                <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className={`text-xs ${stratumColor} opacity-80 flex items-center flex-wrap gap-1.5 mt-0.5`}>
                            {/* 政治光谱小标签 (移至第二行以防重叠) */}
            ...
            <div className="mb-2">
                <div className="text-xs text-gray-500 uppercase tracking-wider mb-1 flex items-center gap-1">
                    ...
                    核心属性
                    {(official.level || 1) > 1 && (
                        <span className="ml-1 px-1 py-0.5 bg-purple-900/50 text-purple-300 rounded text-xs">
                            Lv.{official.level || 1}
                        </span>
                    )}
                    {official.ambition > 50 && (
                        <span className="ml-1 px-1 py-0.5 bg-orange-900/50 text-orange-300 rounded text-xs" title={`野心: ${official.ambition}`}>
                            <Icon name="Flame" size={8} className="inline" /> {official.ambition}
                        </span>
                    )}
                </div>
                <div className="grid grid-cols-4 gap-1">
                    ...
                    <div className="flex flex-col items-center p-1 bg-gray-900/40 rounded border border-blue-800/30">
                        ...
                        <span className="text-xs text-gray-500">行政</span>
                        <span className="text-xs font-bold text-blue-300">{official.stats?.administrative ?? official.administrative ?? 50}</span>
                    </div>
                    {/* 军事 */}
                    <div className="flex flex-col items-center p-1 bg-gray-900/40 rounded border border-red-800/30">
                        ...
                        <span className="text-xs text-gray-500">军事</span>
                        <span className="text-xs font-bold text-red-300">{official.stats?.military ?? official.military ?? 30}</span>
                    </div>
                    {/* 外交 */}
                    <div className="flex flex-col items-center p-1 bg-gray-900/40 rounded border border-green-800/30">
                        ...
                        <span className="text-xs text-gray-500">外交</span>
                        <span className="text-xs font-bold text-green-300">{official.stats?.diplomacy ?? official.diplomacy ?? 30}</span>
                    </div>
                    {/* 威望 */}
                    <div className="flex flex-col items-center p-1 bg-gray-900/40 rounded border border-purple-800/30">
                        ...
                        <span className="text-xs text-gray-500">威望</span>
                        <span className="text-xs font-bold text-purple-300">{official.stats?.prestige ?? official.prestige ?? 50}</span>
                    </div>
                </div>
            </div>
                ...
                <div className="border-r border-gray-700/30 pr-2">
                    ...
                    <div className="space-y-0.5">
                        ...
                        {effectItems.length > 0 ? effectItems : (
                            !official.stratumInfluenceBonus && <div className="text-xs text-gray-500 italic">无显著效果</div>
                        )}
                    </div>
                </div>
                    ...
                    {stance ? (
                        <>
                            ...
                            <div className={`flex items-center gap-1 mb-0.5 ${stanceColors.text}`}>
                                ...
                                <span className="text-xs font-semibold">{stance.name}</span>
                            </div>
                            {/* 政治议题 */}
                            ...
                            {/* 触发条件 - 简化显示 */}
                            <div className="mb-1">
                                <span className="text-xs text-gray-500">政治主张:</span>
                                <div className="text-xs text-gray-300 bg-gray-800/30 p-1.5 rounded border border-gray-700/30 leading-snug break-words mt-0.5">
                            ...
                            {official.stanceActiveEffects && Object.keys(official.stanceActiveEffects).length > 0 && (
                                <div className="mb-0.5">
                                    <span className="text-xs text-green-500 uppercase">满足时:</span>
                            ...
                            {official.stanceUnsatisfiedPenalty && Object.keys(official.stanceUnsatisfiedPenalty).length > 0 && (
                                <div>
                                    <span className="text-xs text-red-500 uppercase">未满足:</span>
                    ...
                    <div className="mt-1 flex flex-wrap gap-1.5">
                        {propertyEntries.slice(0, 4).map(entry => (
                            <span
                                ...
                                className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-gray-800/60 border border-gray-700/60 text-xs text-gray-300"
                            >
                                {entry.buildingName} × {entry.count}
                            </span>
                        ))}
                        {propertyEntries.length > 4 && (
                            <span className="text-xs text-gray-500">等 {propertyEntries.length} 类</span>
                        )}
                    </div>
                </div>
            ) : null}
