...
const MECHANICS_GUIDES = [
    ...
    },
    {
        ...
        content: [
            ...
            {
                type: 'list', items: [
                    '人头税 (Head Tax)：按阶层日均收入的百分比征收。输入的数字就是实际税率（如输入 20 即 20%），收入越高税额越高。过高会降低阶层好感度。',
                    '资源税 (Resource Tax)：对市场交易征收。每当资源在市场上买卖（无论是自动生产消耗还是进出口）时触发。工业发达时这笔收入很可观。',
                    '营业税 (Business Tax)：按建筑营业收入（产值）的百分比征收。输入的数字就是实际税率（如输入 20 即 20%），每种建筑可设置不同税率，由建筑业主支付。',
                    '负税率 (补贴)：你可以将税率设为负数，变成补贴。人头税补贴提升阶层好感度和财富，营业税补贴降低企业生产成本。'
                ]
            },
            { type: 'h4', text: '2.1 营业税详解' },
            { type: 'p', text: '营业税按建筑营业收入（产值）的百分比征收，具有以下特点：' },
            {
                type: 'list', items: [
                    '征收时机：每当建筑产出资源时，自动从业主财产中扣除税款',
                    '按营收征税：税额 = 产值 × 你设置的税率百分比。即使建筑亏损也需缴税',
    ...
    },
    {
        ...
        content: [
            ...
            {
                type: 'list', items: [
                    '触发条件：当战争分数 >= 500 时，可在和平谈判中提出“吞并”',
                    '无条件投降：当战争分数 >= 500 时，敌国会递交投降书，任你处置',
    ...
    },
    {
        ...
        content: [
            ...
            {
                type: 'list', items: [
                    '⚗ 研发知识 +10~+66/项（基础 22，按时代加权）',
                    '🏛 进入新时代 +54~+216（基础 90，按时代加权）',
                    '🏗 建筑里程碑 基础 +30，每达成下一档（50→150→450→…×3）触发一次',
                    '👥 人口里程碑 基础 +34，每达成下一档（100→400→1600→…×4）触发一次',
                    '🚢 贸易里程碑 基础 +22，按 500→1500→4000→10000→25000→60000→150000 累计',
                    '⚔ 战争结果 胜利+39 / 失利+27',
                    '🏭 完成完整产业链 +41/条',
            ...
            {
                type: 'list', items: [
                    '未装备的理念库上限 27：满了之后选择新理念时会进入第二步，提示你放弃 1 个旧的',
                    '卡槽数量随时代逐步解锁：起始 3 个，最多 15 个',
