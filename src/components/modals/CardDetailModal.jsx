  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-[100] flex items-end lg:items-center justify-center">
      ...
      <div className="relative w-full lg:w-auto lg:min-w-[500px] lg:max-w-2xl bg-gray-900 lg:rounded-2xl rounded-t-2xl border-t lg:border border-gray-700 shadow-2xl max-h-[85vh] lg:max-h-[90vh] flex flex-col animate-slide-up lg:animate-fade-in">
        {/* 头部 */}
        <div className="flex items-start justify-between p-3 border-b border-gray-800 bg-gray-800/50">
          <div className="flex items-start gap-2 flex-1">
            {icon && (
              <div className={`p-1.5 rounded-lg ${icon.bgColor || 'bg-gray-700'}`}>
                <Icon name={icon.name} size={20} className={icon.color || 'text-white'} />
              </div>
            )}
            <div className="flex-1 min-w-0">
<h3 className="text-base font-bold text-white font-decorative">{title}</h3>
              {description && (
        ...
        {/* 底部操作按钮 */}
        {actions && (
          <div className="p-3 border-t border-gray-800 bg-gray-800/50 space-y-2">
            {actions.primary && (
              <button
