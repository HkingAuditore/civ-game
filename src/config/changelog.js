...
export const CHANGELOG = [
    {
        ...
        changes: [
            { type: 'balance', text: '全面提升理念分数获取效率：研发知识（基础 15→22，时代加权 8→12）、进入新时代（基础 60→90，时代加权 20→30）、建筑里程碑（40→60）、人口里程碑（45→68）、贸易里程碑（25→38）、战争胜利（50→75）/失利（30→45）、产业链完成（45→68）、成就（20→30，上限 50→75）均上调约 50%。' },
            { type: 'balance', text: '涌现阈值公式由 35+已拥有数×20 下调至 30+已拥有数×14，年度报告理念奖励同步上调（S 级 60→90、A 级 40→60、B 级 25→38、C 级 10→15），整体涌现节奏显著加快。' },
            { type: 'improve', text: '理念卡槽最终上限从 10 提升至 15：时代解锁表扩展至后期时代（蒸汽 10、电气 11、原子 13、信息 15），后期高产出时代可装备更多理念组合。' },
            { type: 'improve', text: '未装备理念库收藏上限从 9 扩容至 27，集卡与构筑空间大幅增加，涌现替换流程（放弃旧理念）触发频率显著降低。' },
        ],
    },
    {
        version: '2.3.62',
    ...
    },
    {
        ...
        changes: [
            { type: 'fix', text: '修复饥荒时自耕农刚被招募就又被晋升抽走的问题：当粮食/布料生产者自家岗位填充率低于 90% 时，禁止其作为晋升来源被抽往高 tier 岗位（如工匠），避免"农田永远 0/150、人口全挤在服务业"的泄漏。' },
            { type: 'fix', text: '修复饥荒判定误用全局满足度导致口粮锁失效的问题：富裕服务业（高工资工匠）会把加权满足度拉高从而误判"无饥荒"；改以生产者自家岗位填充率作为自调节信号，农田/织布坊填满后自动解锁晋升。' },
            { type: 'balance', text: '调整危机转职优先级：饥荒时紧缺资源生产者（如自耕农）迁移门槛大幅下调（最低 0.4×），自耕农作为兜底职业取消收入对比门槛；目标选择时紧缺生产者优先于高收入非紧缺岗位，确保劳力先导向产粮而非涌向矿井/服务业。' },
            { type: 'new', text: '新增饥荒兜底动员机制：全局粮食满足度低于 70% 时，优先将闲置失业者（含本 tick 刚出生人口）直接征召为自耕农填补农田空缺，避免"来了失业者却没去种田就饿死"。' },
            { type: 'balance', text: '严重饥荒（满足度 < 50%）且自耕农严重缺员时，从工匠/商人/神职等非必需服务业按比例（每 tick 最多 30%）紧急抽调劳力下田，打破大崩溃中服务业锁死、单对迁移来不及调人的死亡螺旋。' },
        ],
    },
    {
        version: '2.3.60',
        ...
        isLatest: false,
        highlights: [
            '官员面板新增全部加成汇总，一眼看清净效果',
            '修复高速模式下业主/自耕农岗永远填不满',
            '修复政变叛乱后建筑意外消失的严重问题',
            '业主利润保护上线，地主岗吸引力回归正常',
        ],
        changes: [
            { type: 'new', text: '官员面板新增可折叠的「全部加成汇总」区块：按生产、经济、需求、人口、政治、军事、外交等类别汇总所有在任官员的基础效果、政治立场效果与部长任命加成的净总和，无需逐张点开卡片即可掌握整体加成格局。' },
            { type: 'improve', text: '抽取 effectFormat 共享工具，官员卡片与加成汇总面板复用同一套效果汉化与分类逻辑，避免不同界面出现同一效果不同文案的口径偏差。' },
            { type: 'fix', text: '修复官员政治立场满足状态在列表卡片与详情面板之间可能出现不一致的问题：优先采用模拟端每帧缓存的 isStanceSatisfied 生效值（它才是真正驱动忠诚度增减的口径），仅在缺失时回退实时计算，避免"卡片显示未满足、详情显示已满足"的分歧。' },
            { type: 'fix', text: '修复政变/叛乱处理时可能清空全部建筑的严重 BUG：simulation 在无建筑变动时返回 result.buildings=null（性能优化），旧逻辑将其展开为空对象并写回 state，导致玩家所有建筑消失；现回退到本 tick 喂给 simulation 的 mergedBuildings 作为权威建筑状态。' },
            { type: 'fix', text: '修复高速模式下人口结构（popStructure）与总人口（population）未每 tick 直写 stateRef 的问题：非 UI tick 计算的转职/晋升/生育/死亡变化被丢弃，下一 tick 读到陈旧人口，表现为业主/自耕农岗永远填不满、人口结构整体失真甚至崩溃。' },
            { type: 'fix', text: '修复业主岗位吸引力被严重低估的问题：旧实现按全额市场工资扣减估算业主净利，服务业抬高工资后矿井/庄园业主估算转负，人口判定"当业主不划算"而全涌向工匠/商人/神职；现引入业主利润保护机制（毛利先保留 12% 利润再决定可发工资），与实际发薪口径对齐。' },
            { type: 'fix', text: '修复业主填坑上限（owner-cap）与工资账单不同步的问题：业主不足时多余建筑视为停工、产出已缩减，但雇员工资账单未同步缩减，导致业主"只拿 1 份产出、却付 N 份工资"而严重亏损；现发薪雇员数与计划工资账单均按 ownerFillCapMultiplier 同比缩减。' },
            { type: 'fix', text: '修复业主可负担门槛未按实际持有份额分摊的问题：官员私产/外资/国有部分由各自渠道出资，不应让本国阶层业主为非己有建筑垫付运营现金、连带把整类型产量拖垮；现仅本国阶层业主按 stratumOwnedShare 缩减自筹运营成本。' },
            { type: 'fix', text: '修复严重空缺岗位收入信号被压低导致无人迁入的死循环：少量在岗者因 owner-cap 减产与自身消耗导致实际人均收入极低，整个岗位显得不赚钱；严重空缺（在岗<50%编制）时取实际净收入与按建筑产出估算的潜在收入的较大值，让真正赚钱的空岗能正确吸引迁入。' },
            { type: 'balance', text: '调整转职与晋升参数：严重空缺的高收入业主岗启用快速填补速率与晋升助推（LUCRATIVE_PROMOTION_BOOTSTRAP_RATIO 0.4），严重空缺目标岗位迁移比例提升至 30%/tick，生存危机阈值从 0.5 提高到 0.7（缺口 30% 即可启动应急转职），打破"无业主→停工→无人晋升"与"粮价暴涨却没人下田"的恶性循环。' },
            { type: 'improve', text: '为旧安卓 WebView 补充 Promise.allSettled polyfill（ES2020），避免部分安卓设备启动时因运行时缺失该 API 而白屏。' },
        ],
    },
    {
        version: '2.3.59',
