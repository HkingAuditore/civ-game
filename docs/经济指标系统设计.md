...
PPI篮子权重不再固定，而是**根据实际生产数据动态计算**：

```javascript
// 动态权重计算（基于最近30天的生产数据）
dynamicPPIBasket = calculateDynamicPPIBasket({
  supplyBreakdownHistory,  // 最近30天的生产数据
  marketPrices,
  equilibriumPrices,
  window: 30,              // 使用30天滚动平均
});
...
supplyBreakdownHistory = [
  // Day 1
  {
    food: { buildings: { farm1: 100, farm2: 150 }, imports: 50 },
    wood: { buildings: { lumberCamp1: 80 }, imports: 0 },
    iron: { buildings: { mine1: 30, mine2: 40 }, imports: 20 },
  },
  // Day 2
  { /* ... */ },
  // ... 最近30天
...
1. **统计平均生产量**：
```javascript
avgProduction[resource] = Σ(最近30天的建筑产量) / 30
```

2. **计算生产价值**：
```javascript
productionValue[resource] = avgProduction[resource] × price[resource]
```

3. **计算权重**：
```javascript
weight[resource] = productionValue[resource] / Σ(所有资源的生产价值)
```
...
**计算方法**：与CPI相同，但使用动态生产者篮子

**数据来源**：
- `supplyBreakdownHistory`: 最近30天的生产数据历史
- `marketPrices`: 当前市场价格
...
| v1.1 | 2026-02-04 | 实现动态PPI篮子，基于实际生产数据自动调整权重 |
